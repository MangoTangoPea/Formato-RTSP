#!/usr/bin/env python3
"""
Emisor RTSP para Intel RealSense D435 — Versión Linux/Ubuntu.
...
Uso:
    python3 emisor_ubuntu.py [--puerto PUERTO] [--cam INDICE] [--calidad BITRATE_KBPS]
    python3 emisor_ubuntu.py --listar-camaras

Ejemplo:
    python3 emisor_ubuntu.py --puerto 8554 --cam 0 --calidad 4000
"""
...
try:
    ...
    print("  En algunas distribuciones puede requerir compilación desde fuente.")
    print("  Ver: https://github.com/IntelRealSense/librealsense")
    sys.exit(1)
...
# URL de descarga de MediaMTX para Linux (versión estable)
MEDIAMTX_VERSION = "v1.12.2"

# Detectar arquitectura del sistema
_ARCH = platform.machine().lower()
if _ARCH in ("x86_64", "amd64"):
    _MEDIAMTX_ARCH = "amd64"
elif _ARCH in ("aarch64", "arm64"):
    _MEDIAMTX_ARCH = "arm64v8"
elif _ARCH.startswith("arm"):
    _MEDIAMTX_ARCH = "armv7"
else:
    _MEDIAMTX_ARCH = "amd64"  # Fallback

MEDIAMTX_URL = (
    f"https://github.com/bluenviron/mediamtx/releases/download/"
    f"{MEDIAMTX_VERSION}/mediamtx_{MEDIAMTX_VERSION}_linux_{_MEDIAMTX_ARCH}.tar.gz"
)
...
def descargar_mediamtx():
    """
    Descarga y extrae el servidor MediaMTX para Linux.
    ...
    print(f"  ↓ Descargando MediaMTX {MEDIAMTX_VERSION} para Linux ({_MEDIAMTX_ARCH}) ...")
    print(f"    URL: {MEDIAMTX_URL}")

    os.makedirs(DIR_MEDIAMTX, exist_ok=True)
    tar_path = os.path.join(DIR_MEDIAMTX, "mediamtx.tar.gz")
    try:
        urllib.request.urlretrieve(MEDIAMTX_URL, tar_path)
    ...
    try:
        with tarfile.open(tar_path, "r:gz") as tar_ref:
            tar_ref.extractall(DIR_MEDIAMTX)
    except tarfile.TarError:
        print("  ✗ El archivo descargado no es un TAR.GZ válido.")
        os.remove(tar_path)
        sys.exit(1)
    ...
    if os.path.isfile(exe_path):
        ...
        print("  ✗ No se encontró el binario 'mediamtx' después de la extracción.")
        sys.exit(1)
...
def iniciar_mediamtx(puerto):
    ...
    try:
        ...
        print("    Intenta:  chmod +x " + exe_path)
        sys.exit(1)
    except Exception as e:
...
def listar_camaras_realsense():
    ...
    if len(dispositivos) == 0:
        ...
        print("    En Linux, verifica permisos con:  ls -l /dev/video*")
        print("    Puede requerir reglas udev para RealSense:")
    ...
    print("  " + "─" * 56)
    print(f"\n  Uso: python3 emisor_ubuntu.py --cam <índice>")
    return lista
...
def iniciar_emisor(indice_camara=0, puerto=PUERTO_RTSP_DEFECTO, bitrate_kbps=2000):
    ...
    try:
        ...
        print("\n" + "═" * 60)
        print("  EMISOR RTSP RealSense — Linux/Ubuntu (4 Streams)")
        print("═" * 60)
        ...
        try:
            ...
            print("    Asegúrate de que la cámara esté conectada por USB 3.0.")
            print("    En Linux, verifica permisos USB y reglas udev.")
            sys.exit(1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Emisor RTSP (Linux/Ubuntu): transmite vídeo RGB + Profundidad + Infrarrojos de una cámara Intel RealSense D435.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Ejemplos de uso:
  python3 emisor_ubuntu.py                          # Cámara 0, puerto 8554, 2000 kbps
  python3 emisor_ubuntu.py --cam 1                  # Usar segunda cámara RealSense
  python3 emisor_ubuntu.py --puerto 9554            # Usar puerto alternativo
  python3 emisor_ubuntu.py --calidad 4000           # Mayor calidad de vídeo
  python3 emisor_ubuntu.py --listar-camaras         # Ver cámaras RealSense detectadas
  python3 emisor_ubuntu.py --cam 0 --puerto 8554    # Todos los parámetros explícitos
